...
extern vec3 falloff = vec3(0.4, 1.0, 20.0);

extern vec2 uv_offsets;
...
vec4 position(mat4 transform_projection, vec4 vertex_position) {
  ...
  // vertex_normal = vertex_position.xyz;
...
#ifdef PIXEL
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
  texture_coords += uv_offsets;

  //RGBA of our diffuse color
  vec4 DiffuseColor = texture2D(texture, texture_coords);

  //RGB of our normal map
  // vec3 NormalMap = (texture2D(texture, texture_coords).rgb + vertex_normal) / 2;
  vec3 NormalMap = vertex_normal;
  // vec3 NormalMap = texture2D(texture, texture_coords).rgb;
